    ...
    <main className="min-h-screen bg-[#0b1f3d] text-slate-50">
      <div className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-10">
        <header className="rounded-2xl border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-slate-900/50 backdrop-blur">
          <p className="text-xs uppercase tracking-[0.3em] text-slate-300">Movimiento de Almacén</p>
          <h1 className="text-3xl font-semibold">Transferencia de Productos</h1>
          <p className="text-sm text-slate-300">ID: {id}</p>
          <p className="text-sm text-slate-300">Origen: {transfer.from}</p>
          <p className="text-sm text-slate-300">Destino: {transfer.to}</p>
          <p className="text-sm text-slate-300">Ítems transferidos: {transfer.items}</p>
