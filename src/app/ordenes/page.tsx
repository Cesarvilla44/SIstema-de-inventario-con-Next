    ...
    <main className={`min-h-screen ${theme === "dark" ? "bg-[#0b1f3d] text-slate-50" : "bg-slate-100 text-slate-900"}`}>
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10">
        <header className={`rounded-2xl border p-6 shadow-2xl backdrop-blur ${theme === "dark" ? "border-white/10 bg-slate-900/70 shadow-slate-900/50" : "border-slate-200 bg-white shadow-slate-200/50"}`}>
          <p className={`text-xs uppercase tracking-[0.3em] ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>Órdenes</p>
          <h1 className={`text-3xl font-semibold ${theme === "dark" ? "text-white" : "text-slate-900"}`}>Gestión de órdenes</h1>
          <p className={`text-sm ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>Órdenes reales con alta/baja.</p>
        ...
        <section className={`rounded-2xl border p-4 shadow-xl ${theme === "dark" ? "border-white/10 bg-slate-900/70 shadow-slate-900/50" : "border-slate-200 bg-white shadow-slate-200/50"}`}>
          <div className="mb-3 flex items-center justify-between">
            <h2 className={`text-lg font-semibold ${theme === "dark" ? "text-white" : "text-slate-900"}`}>Órdenes recientes</h2>
            ...
            <table className={`min-w-full text-sm ${theme === "dark" ? "bg-slate-950/60" : "bg-white"}`}>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  orders.map((o) => (
                    <tr key={o.id} className={`border-b ${theme === "dark" ? "border-white/10 hover:bg-white/5" : "border-slate-200 hover:bg-slate-50"}`}>
                      ...
                      <td className="px-3 py-2">
                        <Link className={`hover:underline font-medium ${theme === "dark" ? "text-white" : "text-slate-900"}`} href={`/ordenes/${o.id}`}>
                          {o.customer}
                        </Link>
                      </td>
                      <td className="px-3 py-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className={`w-full max-w-lg rounded-2xl border p-6 shadow-2xl backdrop-blur ${theme === "dark" ? "border-white/10 bg-slate-900/90 shadow-slate-950/60" : "border-slate-200 bg-white shadow-slate-200/60"}`}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className={`text-xs uppercase tracking-[0.3em] ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>Nueva orden</p>
                <h3 className={`text-xl font-semibold ${theme === "dark" ? "text-white" : "text-slate-900"}`}>Crear orden</h3>
              </div>
            ...
            <div className="mt-4 space-y-3">
              <div className="space-y-1">
                <Label className={theme === "dark" ? "" : "text-slate-700"}>Cliente</Label>
                <Input value={customer} onChange={(e) => setCustomer(e.target.value)} placeholder="Nombre del cliente" className={theme === "dark" ? "" : "bg-slate-50 border-slate-300"} />
              </div>
              <div className="space-y-1">
                <Label className={theme === "dark" ? "" : "text-slate-700"}>Estado</Label>
                <Input value={status} onChange={(e) => setStatus(e.target.value)} placeholder="Pendiente / Procesado" className={theme === "dark" ? "" : "bg-slate-50 border-slate-300"} />
              </div>
              <div className="space-y-1">
                <Label className={theme === "dark" ? "" : "text-slate-700"}>Monto</Label>
                <Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0" className={theme === "dark" ? "" : "bg-slate-50 border-slate-300"} />
              </div>
              <div className="space-y-1">
